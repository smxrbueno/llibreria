<?php
   ...
   if($conn){
    ...
    $consulta= "select nom from Llibre WHERE autor like '$autor'" ; //guardem la consulta que realitzarem en la variable consulta
    ...
    if(mysqli_num_rows($Result)>0) //si ens retorna més de 0 files...
    {
       ...
       {
            if($row['nom']!=null){ //si la fila retornada de la consulta es diferent de null
                echo "<p>" .$row['nom']."<p>"; //la imprimim fent un salt de linia
        ...
        echo "No s'ha pogut agafar el nom del llibre";
    }
